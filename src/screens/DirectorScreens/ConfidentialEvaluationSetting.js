...
import EvaluationPinService from '../Services/EvaluationPinServices';
  ...
  const handleSave = async () => {
    try {
      ...
      const evaluationTime = {
        ...
        end_time: endTime,
        evaluation_type: 'confidential',
        session_id: sid.id,
      };

      console.log('Sending evaluation time data to server:', evaluationTime);

      const pinResponse = await EvaluationPinService.postConfidentialEvaluationPin(evaluationPin);
      const timeResponse = await postEvaluationTime(evaluationTime);

      console.log(`Pin Response from server: ${JSON.stringify(pinResponse)}`);
      console.log(`Time Response from server: ${JSON.stringify(timeResponse)}`);

      if ((pinResponse && pinResponse.id) && (timeResponse && timeResponse.id)) {
        Alert.alert('Success', 'Evaluation time saved successfully.');
      } else {
        Alert.alert('Error', 'Something went wrong with saving evaluation time or pin.');
      }
    } catch (error) {
      console.error(error);
